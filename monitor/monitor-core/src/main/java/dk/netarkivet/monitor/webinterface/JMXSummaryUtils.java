...
/**
 * Various utility methods and classes for the JMX Monitor page. and a bunch of JMX properties used by
...
public class JMXSummaryUtils {
    ...
    public static final String JMXPhysLocationProperty = dk.netarkivet.common.management.Constants.PRIORITY_KEY_LOCATION;
    /** JMX property for the machine name. */
    public static final String JMXMachineNameProperty = dk.netarkivet.common.management.Constants.PRIORITY_KEY_MACHINE;
    /** JMX property for the application name. */
    public static final String JMXApplicationNameProperty = dk.netarkivet.common.management.Constants.PRIORITY_KEY_APPLICATIONNAME;
    /** JMX property for the application instance id. */
    public static final String JMXApplicationInstIdProperty = dk.netarkivet.common.management.Constants.PRIORITY_KEY_APPLICATIONINSTANCEID;
    /** JMX property for the HTTP port. */
    public static final String JMXHttpportProperty = dk.netarkivet.common.management.Constants.PRIORITY_KEY_HTTP_PORT;
    /** JMX property for the harvest channel. */
    public static final String JMXHarvestChannelProperty = dk.netarkivet.common.management.Constants.PRIORITY_KEY_CHANNEL;
    /** JMX property for the replica name. */
    public static final String JMXArchiveReplicaNameProperty = dk.netarkivet.common.management.Constants.PRIORITY_KEY_REPLICANAME;
    /** JMX property for the index. */
    public static final String JMXIndexProperty = dk.netarkivet.common.management.Constants.PRIORITY_KEY_INDEX;
    ...
    public static final String[] STARRABLE_PARAMETERS = new String[] {JMXRemoveApplication, JMXPhysLocationProperty,
            JMXMachineNameProperty, JMXApplicationNameProperty, JMXApplicationInstIdProperty, JMXHttpportProperty,
            JMXHarvestChannelProperty, JMXArchiveReplicaNameProperty, JMXIndexProperty};
     ...
     *
     * @param applicationName The class name of the application, should not be null.
    ...
    public static String reduceApplicationName(String applicationName) throws ArgumentNotValid {
        ArgumentNotValid.checkNotNull(applicationName, "String applicationName");
     ...
     * @throws ArgumentNotValid if argument isn't valid.
     */
    public static String generateShowColumn(StarredRequest starredRequest, Locale l) throws ArgumentNotValid {
     ...
     * restricted. This function is only used by JMXIndexProperty field, the other properties uses generateShowLing
     * instead.
     *
     ...
     */
    public static String generateShowAllLink(StarredRequest starredRequest, String parameter, Locale l)
        ...
        if (CHARACTER_SHOW_ALL.equals(starredRequest.getParameter(parameter))) {
            ...
            return "(" + generateLink(starredRequest, parameter, CHARACTER_SHOW_ALL, I18N.getString(l, "showall"))
                    + ")";
        }
    }
     ...
     * @throws ArgumentNotValid if arguments isn't valid.
     */
    public static String generateShowLink(StarredRequest starredRequest, String parameter, Locale l)
            throws ArgumentNotValid {
        ArgumentNotValid.checkNotNull(starredRequest, "StarredRequest starredRequest");
        ArgumentNotValid.checkNotNull(parameter, "String parameter");
        if (CHARACTER_SHOW_ALL.equals(starredRequest.getParameter(parameter))) {
            return "(" + generateLink(starredRequest, parameter, CHARACTER_NOT_COLUMN, I18N.getString(l, "hide")) + ")";
        } else {
            return "(" + generateLink(starredRequest, parameter, CHARACTER_SHOW_ALL, I18N.getString(l, "showall"))
                    + ", " + generateLink(starredRequest, parameter, CHARACTER_NOT_COLUMN, I18N.getString(l, "hide"))
                    + ")";
        }
    }
     ...
     * @throws ArgumentNotValid if argument isn't valid.
     */
    public static boolean showColumn(StarredRequest starredRequest, String parameter) throws ArgumentNotValid {
        ArgumentNotValid.checkNotNull(starredRequest, "StarredRequest starredRequest");
     ...
     *
     * @param request A request to draw other parameter values from, should not be null.
    ...
    public static String generateLink(StarredRequest request, String setPart, String setValue, String linkText)
        ...
        for (String queryPart : STARRABLE_PARAMETERS) {
            ...
            if (queryPart.equals(setPart)) {
                ...
                builder.append(HTMLUtils.encode(request.getParameter(queryPart)));
            }
        }
        builder.append("\">");
     ...
     */
    public static List<StatusEntry> queryJMXFromRequest(String[] parameters, StarredRequest request, PageContext context)
        ...
        try {
            ...
            if (query != null) {
                HTMLUtils.forwardWithErrorMessage(context, I18N, e, "errormsg;error.in.querying.jmx.with.query.0",
                        query);
                throw new ForwardedToErrorPage("Error in querying JMX with query '" + query + "'.", e);
            } else {
                HTMLUtils.forwardWithErrorMessage(context, I18N, e, "errormsg;error.in.building.jmxquery");
     ...
     * @throws ArgumentNotValid if arguments isn't valid.
     */
    public static void unregisterJMXInstance(String[] parameters, StarredRequest request, PageContext context)
            throws ArgumentNotValid {
        ArgumentNotValid.checkNotNull(parameters, "String[] parameters");
        ArgumentNotValid.checkNotNull(context, "PageContext context");
        String query = null;
        try {
            ...
            // are treated equal.
            HTMLUtils.forwardWithErrorMessage(context, I18N, e, "errormsg;error.when.unregistering.mbean.0", query);
            throw new ForwardedToErrorPage("Error when unregistering JMX MBean with query '" + query + "'.", e);
        }
    }
     ...
     */
    public static String createJMXQuery(String[] parameters, StarredRequest starredRequest) throws ArgumentNotValid {
        ArgumentNotValid.checkNotNull(parameters, "String[] parameters");
        ArgumentNotValid.checkNotNull(starredRequest, "StarredRequest starredRequest");
        StringBuilder query = new StringBuilder(LOGGING_MBEAN_NAME_PREFIX + "*");
        for (String queryPart : parameters) {
            if (!CHARACTER_SHOW_ALL.equals(starredRequest.getParameter(queryPart))
                    && !CHARACTER_NOT_COLUMN.equals(starredRequest.getParameter(queryPart))) {
                query.append(",");
    ...
    /**
     * Make an HTML fragment that shows a log message preformatted. If the log message is longer than
     * NUMBER_OF_LOG_LINES lines, the rest are hidden and replaced with an internationalized link "More..." that will
     * show the rest. Long loglines are split to length approximately monitor.preferredMaxJMXLogLength and then split
     * again to ensure that no lines are wider than monitor.absoluteMaxJMXLogLength. However as the wrapping algorithm
     * is not sophisticated it is better to split the lines in a meaningful way where they are generated.
     *
     ...
     * @throws ArgumentNotValid if argument isn't valid.
     */
    public static String generateMessage(String logMessage, Locale l) throws ArgumentNotValid {
        ...
        logMessage = StringUtils.splitStringOnWhitespace(logMessage, PREFERRED_LENGTH);
        logMessage = StringUtils.splitStringForce(logMessage, MAX_LENGTH);
        BufferedReader sr = new BufferedReader(new StringReader(logMessage));
        msg.append("<pre>");
        int lineno = 0;
        String line;
        try {
            while (lineno < NUMBER_OF_LOG_LINES && (line = sr.readLine()) != null) {
                msg.append(line);
                msg.append('\n');
                ++lineno;
            }
            ...
            if ((line = sr.readLine()) != null) {
                ...
                msg.append(id);
                msg.append("').style.display='block';" + "document.getElementById('show");
                msg.append(id);
