...
PIDS=$(ps -wwfe | grep dk.netarkivet.harvester.heritrix3.HarvestControllerApplication | grep -v grep | grep /home/test/QUICKSTART/conf/settings_HarvestControllerApplication_snapshot1.xml | awk "{print \$2}")
if [ -n "$PIDS" ] ; then
    ...
    java -Xmx1536m  -Ddk.netarkivet.settings.file=/home/test/QUICKSTART/conf/settings_HarvestControllerApplication_snapshot1.xml -Dlogback.configurationFile=/home/test/QUICKSTART/conf/logback_HarvestControllerApplication_snapshot1.xml dk.netarkivet.harvester.heritrix3.HarvestControllerApplication < /dev/null > start_HarvestControllerApplication_snapshot1.log 2>&1 &
fi
